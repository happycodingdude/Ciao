...
const MessageContent_RW = (props) => {
  ...
  return (
    ...
    >
      ...
      <div
        className={`relative flex flex-col laptop:w-[clamp(60rem,70%,90rem)] desktop:w-[clamp(40rem,70%,80rem)] ${message.contactId === info.id ? "items-end" : "items-start"}`}
      >
        {/* Sender infor */}
        <div
        ...
        {message.attachments && message.attachments.length !== 0 ? (
          message.attachments?.length <= 2 ? (
            ...
            <div
              className={`flex w-full  ${message.contactId === info.id ? "justify-end" : ""} gap-[.5rem]`}
            >
              <ImageWithLightBoxAndNoLazy
                src={message.attachments[0].mediaUrl}
                title={message.attachments[0].mediaName.split(".")[0]}
                className={`loaded  aspect-square !w-[50%]  cursor-pointer !bg-[size:100%]`}
                slides={message.attachments.map((item) => ({
                  src: item.type === "image" ? item.mediaUrl : "",
                }))}
                index={0}
              />
              <div className="flex w-[30%] flex-col gap-[.5rem]">
                <ImageWithLightBoxAndNoLazy
                  src={message.attachments[1].mediaUrl}
                  title={message.attachments[1].mediaName?.split(".")[0]}
                  className={`loaded  aspect-square w-full  cursor-pointer !bg-[size:120%]`}
                  slides={message.attachments.map((item) => ({
                    src: item.type === "image" ? item.mediaUrl : "",
                  }))}
                  index={1}
                ...
                <ImageWithLightBoxAndNoLazy
                  ...
                  index={2}
                />
                {/* {message.attachments.slice(1, 3).map((item, index) => {
                  return (
                    <ImageWithLightBoxAndNoLazy
                      src={item.mediaUrl}
                      title={item.mediaName?.split(".")[0]}
...
export default MessageContent_RW;
